...
N.randomvec <- 100
ncores <- 40
ibd <- get(load(paste0(cm,"cm/",runs,"/all_",cm,".max.global.cut0.088.RData")))
ibd <- ibd[rownames(ibd) %in% ids, colnames(ibd) %in% ids]
IBD.chol <- chol(ibd)
go_on <- TRUE
while(go_on) {
...
gc()
ibd.file.list <- list.files(paste0(cm,"cm/",runs,"/chunk.1cM"), pattern = ".RData", full.names = TRUE)
t1 <- proc.time()
out <- ibd_mapping_batch(obj1, offset = offset, ibd.ids = ids, ibd.file.list = ibd.file.list, ncores = ncores)
t2 <- proc.time()
write.table(out, paste0("seed", seed, "_",cm,"cm_run",runs,".txt"), quote=F, row.names=F, col.names=T)
write.table(t(c(t2-t1)), paste0("seed", seed, "_",cm,"cm_run",runs,".time"), quote=F, row.names=F, col.names=T)
