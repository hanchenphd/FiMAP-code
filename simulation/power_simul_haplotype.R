##### randomly select haplotype clusters and assign effect sizes
...
maf2 <- 0.01
lengths <- read.table("data/UKB/blockPBWT_0.5cm_500/file.lengths.txt")[,1]
...
for(i in 1:nsimul) {
    print(i)
    chr <- sample(22, 1)
    ii <- sample(lengths[chr], 1)
    data <- scan(pipe(paste0("sed -ne '",ii,"p' data/UKB/blockPBWT_0.5cm_500/chr",chr,"_block_0.5.blocks")), what = "character")
    start <- as.numeric(data[3])
    end <- as.numeric(data[4])
    start1 <- floor(start)-1
    start2 <- ceiling(end)
    start <- if(round(start) == round(end) && round(start) - start > end - round(end)) round(start) - 1 else round(start)
    data.list <- strsplit(data[-(1:5)], "-")
    data <- unlist(lapply(data.list, function(x) as.numeric(x[1])))
    ...
    data <- match(data, sample.id)
    data <- data[!is.na(data)]
    geno <- rep(0, length(sample.id))
    geno[data[!duplicated(data)]]<-geno[data[!duplicated(data)]]+1
    geno[data[duplicated(data)]]<-geno[data[duplicated(data)]]+1
    beta <- sqrt(rsq/var(geno))
    geneffects <- geno * beta
    ...
    glmm.score(obj3, infile = gds.file, outfile = paste0("haplotype/rsq_",rsq,"_seed", seed, ".GWAS.simul",i,".chr",chr,".start",start,"cM.txt"), MAF.range = c(maf2, 0.5), miss.cutoff = 0.05, ncores = ncores)
    svtres <- read.table(paste0("haplotype/rsq_",rsq,"_seed", seed, ".GWAS.simul",i,".chr",chr,".start",start,"cM.txt"), header=T, as.is=T)
    out$p.svt.min <- min(svtres$PVAL)
    out$p.svt.cauchy <- pcauchy(mean(qcauchy(svtres$PVAL,lower=F)),lower=F)
    unlink(paste0("haplotype/rsq_",rsq,"_seed", seed, ".GWAS.simul",i,".chr",chr,".start",start,"cM.txt"))
    glmm.score(obj3, infile = gds2.file, outfile = paste0("haplotype/rsq_",rsq,"_seed", seed, ".GWAS.simul",i,".chr",chr,".start",start,"cM.txt"), MAF.range = c(maf2, 0.5), miss.cutoff = 0.05, ncores = ncores)
    svtres <- read.table(paste0("haplotype/rsq_",rsq,"_seed", seed, ".GWAS.simul",i,".chr",chr,".start",start,"cM.txt"), header=T, as.is=T)
    out$p.svt.imputed.min <- min(svtres$PVAL)
    out$p.svt.imputed.cauchy <- pcauchy(mean(qcauchy(svtres$PVAL,lower=F)),lower=F)
    unlink(paste0("haplotype/rsq_",rsq,"_seed", seed, ".GWAS.simul",i,".chr",chr,".start",start,"cM.txt"))
    if(file1) {
    glmm.score(obj3, infile = gds.file1, outfile = paste0("haplotype/rsq_",rsq,"_seed", seed, ".GWAS.simul",i,".chr",chr,".start",start1,"cM.txt"), MAF.range = c(maf2, 0.5), miss.cutoff = 0.05, ncores = ncores)
    svtres <- read.table(paste0("haplotype/rsq_",rsq,"_seed", seed, ".GWAS.simul",i,".chr",chr,".start",start1,"cM.txt"), header=T, as.is=T)
    out$p.svt.min1 <- min(svtres$PVAL)
    out$p.svt.cauchy1 <- pcauchy(mean(qcauchy(svtres$PVAL,lower=F)),lower=F)
    unlink(paste0("haplotype/rsq_",rsq,"_seed", seed, ".GWAS.simul",i,".chr",chr,".start",start1,"cM.txt"))
    glmm.score(obj3, infile = gds2.file1, outfile = paste0("haplotype/rsq_",rsq,"_seed", seed, ".GWAS.simul",i,".chr",chr,".start",start1,"cM.txt"), MAF.range = c(maf2, 0.5), miss.cutoff = 0.05, ncores = ncores)
    svtres <- read.table(paste0("haplotype/rsq_",rsq,"_seed", seed, ".GWAS.simul",i,".chr",chr,".start",start1,"cM.txt"), header=T, as.is=T)
    ...
    if(file2) {
    glmm.score(obj3, infile = gds.file2, outfile = paste0("haplotype/rsq_",rsq,"_seed", seed, ".GWAS.simul",i,".chr",chr,".start",start2,"cM.txt"), MAF.range = c(maf2, 0.5), miss.cutoff = 0.05, ncores = ncores)
    svtres <- read.table(paste0("haplotype/rsq_",rsq,"_seed", seed, ".GWAS.simul",i,".chr",chr,".start",start2,"cM.txt"), header=T, as.is=T)
    out$p.svt.min2 <- min(svtres$PVAL)
    out$p.svt.cauchy2 <- pcauchy(mean(qcauchy(svtres$PVAL,lower=F)),lower=F)
    unlink(paste0("haplotype/rsq_",rsq,"_seed", seed, ".GWAS.simul",i,".chr",chr,".start",start2,"cM.txt"))
    glmm.score(obj3, infile = gds2.file2, outfile = paste0("haplotype/rsq_",rsq,"_seed", seed, ".GWAS.simul",i,".chr",chr,".start",start2,"cM.txt"), MAF.range = c(maf2, 0.5), miss.cutoff = 0.05, ncores = ncores)
    svtres <- read.table(paste0("haplotype/rsq_",rsq,"_seed", seed, ".GWAS.simul",i,".chr",chr,".start",start2,"cM.txt"), header=T, as.is=T)
...
write.table(res, paste0("haplotype/rsq_",rsq,"_seed", seed, ".txt"), quote=F, row.names=F, col.names=T)
