...
N.randomvec <- as.numeric(cmd[6]) # 100, 1000, 10000
cat("Number of random vectors:", N.randomvec, "\n")
label <- cmd[7] # 10k, 100k, 1m, 10m
cat("Label:", label, "\n")
...
t2 <- proc.time()
write.table(out, paste0(outfile.dir, "seed", seed, "_random", N, "_simIBD_",label,"_B", N.randomvec, "_", pheno.name, ".txt"), quote=F, row.names=F, col.names=T)
write.table(t(c(t2-t1)), paste0(outfile.dir, "seed", seed, "_random", N, "_simIBD_",label,"_B", N.randomvec, "_", pheno.name, ".time"), quote=F, row.names=F, col.names=T)
