...
cat("RaPID", cm, "cm...\n")
runs <- cmd[4] # 1
cat("Using RaPID results from replicate", runs, "...\n")
ncores <- as.numeric(cmd[5]) # 40
cat("Using", ncores, "cores...\n")
label <- cmd[6] # 10k, 100k, 1m, 10m
cat("Label:", label, "\n")
if(label == "10k") B <- 10000
if(label == "100k") B <- 100000
if(label == "1m") B <- 1000000
if(label == "10m") B <- 10000000
...
ibd.file.list <- list.files(local.ibd.dir, pattern = "chr\\d+.start\\d+cM.RData", full.names = TRUE)
...
set.seed(seed)
pheno <- pheno[sample(1:nrow(pheno), N), ]
...
Sigma.sqrt <- crossprod(t(Sigma.eigen$vectors)*sqrt(Sigma.eigen$values),t(Sigma.eigen$vectors))
P1 <- tcrossprod(obj$Sigma_i, Sigma.sqrt) - tcrossprod(obj$Sigma_iX, tcrossprod(crossprod(Sigma.sqrt, obj$Sigma_iX), obj$cov))
rm(Sigma.eigen, Sigma.sqrt)
obj1 <- list(theta = obj$theta, scaled.residuals = obj$scaled.residuals, random.vectors = sqrt(N)*as.matrix(P1), ncovar = ncol(obj$X), id_include = obj$id_include)
class(obj1) <- "glmmkin.randomvec"
offset <- crossprod(obj1$random.vectors, crossprod(Sigma, obj1$random.vectors))
rm(obj, tmpibd, Sigma, P1)
out <- ibd_mapping_batch(obj1, offset = offset, ibd.ids = pheno$eid, ibd.file.list = ibd.file.list, ncores = ncores)
t2 <- proc.time()
write.table(out, paste0(outfile.dir, "seed", seed, "_random", N, "_simIBD_",label,"_exact_", pheno.name, ".txt"), quote=F, row.names=F, col.names=T)
write.table(t(c(t2-t1)), paste0(outfile.dir, "seed", seed, "_random", N, "_simIBD_",label,"_exact_", pheno.name, ".time"), quote=F, row.names=F, col.names=T)
