...
seed <- as.numeric(cmd[2]) # 12345 54321
cat("Using seed", seed, "...\n")
cm <- cmd[3] # 3,5,10
cat("RaPID", cm, "cm...\n")
runs <- cmd[4] # 1 2
...
t2 <- proc.time()
write.table(out, paste0(outfile.dir, "seed", seed, "_", pheno.name, ".txt"), quote=F, row.names=F, col.names=T)
write.table(t(c(t2-t1)), paste0(outfile.dir, "seed", seed, "_", pheno.name, ".time"), quote=F, row.names=F, col.names=T)
