# Fig 6. GWAS and finite-sample FiMAP p values from RaPID IBD segments on chromosome 21.
...
infile <- paste0("GWAS_imputed.chr",CHR,".X50.0.0.QUALgt0.3.txt")
...
infile <- "3cm/1/seed12345_X50.0.0.txt"
...
infile <- paste0("3cm/1/conditional_imputed_GDS_pheno50_maf1e-04_missrate0.05_qual0.3_flank3cM_alpha",alpha,".out")
...
data$p.value[data$p.value < pval.cutoff2] <- pval.cutoff2
data <- data[order(data$chr, data$pos),]
ann <- factor(data$source, levels = 1:3, labels = c("", "tmp1", "tmp2"))
p3 <- manhattan.plot(data$chr, data$pos, data$p.value, sig.level = 0.05/3403, annotate=list(ann,"tmp1"=list(col=cols[6],pch=16,cex=1.5,label=F),"tmp2"=list(col=cols[5],pch=17,cex=1,label=F)),xlab=list(label="Standing height",cex=2.5),ylab=list(label=expression(-log[10](italic(p))),cex=2.5))

infile <- paste0("GWAS_imputed.chr",CHR,".X20015.0.0.QUALgt0.3.txt")
data <- read.table(infile, header=T, as.is=T)
data <- subset(data, AF >= maf & AF <= 1 - maf)
data <- subset(data, MISSRATE < missrate)
data <- subset(data, QUAL > qual) # imputed
...
infile <- "3cm/1/seed12345_X20015.0.0.txt"
data2 <- read.table(infile, header=T)
data2 <- subset(data2, chr == CHR)
data2$pos <- (data2$start+data2$end)/2
data2 <- data2[, c("chr", "pos", "p.value")]
...
infile <- paste0("3cm/1/conditional_imputed_GDS_pheno20015_maf1e-04_missrate0.05_qual0.3_flank3cM_alpha",alpha,".out")
data3 <- read.table(infile, header=T, as.is=T, sep="\t")
data3 <- subset(data3, chr == CHR)
data3$pos <- (data3$start+data3$end)/2
data3 <- data3[, c("chr", "pos", "p.value.conditional")]
...
data <- data[order(data$chr, data$pos),]
ann <- factor(data$source, levels = 1:3, labels = c("", "tmp1", "tmp2"))
p4 <- manhattan.plot(data$chr, data$pos, data$p.value, sig.level = 0.05/3403, annotate=list(ann,"tmp1"=list(col=cols[6],pch=16,cex=1.5,label=F),"tmp2"=list(col=cols[5],pch=17,cex=1,label=F)),xlab=list(label="Sitting height",cex=2.5),ylab=list(label=expression(-log[10](italic(p))),cex=2.5))

plot_grid(p3, p4, ncol = 1, align = "hv", labels = "AUTO", label_size=30)
ggsave("Fig6.eps", width=12, height=8)
